...
export interface FunnelStats {
  ...
  screenedIneligible: number;
  donated: number;
  noShow: number;
  conversionRate: number; // (donated / registered) * 100
  attendanceRate: number; // (checkedIn / registered) * 100
}
...
export const getDashboardAnalytics = async (): Promise<DashboardStats> => {
  const [
    ...
    BloodDonationEvent.countDocuments(),
    BloodDonationEvent.countDocuments({ status: { $in: ['open', 'upcoming'] } }),
    Registration.countDocuments({ donationStatus: { $ne: 'cancelled' } }),
    ...
    BloodDonationEvent.find().sort({ startDate: -1 }).limit(6).lean(),
    Registration.find()
  ...
  const completionRate =
    fData.registered > 0 ? Math.round((totalDonated / fData.registered) * 100) : 0;
...
export const getEventFunnelAnalytics = async (eventId: string): Promise<FunnelStats> => {
...
export const generateEventCSV = (event: any, registrations: any[]): string => {
  const BOM = '\uFEFF';
  const headers = [
    'Mã Đăng Ký',
    'Họ Và Tên',
  ...
  const rows = registrations.map((r) => {
    ...
    const registeredDate = r.registeredAt
      ...
      : '';
    const screening =
      r.screeningResult?.doctorConclusion === 'eligible'
        ? 'Đủ điều kiện'
    ...
    const isCheckedIn =
      r.checkIn?.status === 'checked_in' || r.donationStatus === 'donated'
        ? 'Đã có mặt'
        : 'Chưa có mặt';
