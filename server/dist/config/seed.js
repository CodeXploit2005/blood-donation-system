...
exports.seedDatabase = exports.normalizeExistingData = void 0;
...
const normalizeExistingData = async () => {
    try {
        ...
        await Registration_1.Registration.updateMany({ donationStatus: { $exists: false } }, [
            {
                $set: {
                    donationStatus: {
                        $cond: [
        ...
        await Registration_1.Registration.updateMany({ 'checkIn.status': true }, { $set: { 'checkIn.status': 'checked_in', donationStatus: 'donated' } });
        await Registration_1.Registration.updateMany({ 'checkIn.status': false }, { $set: { 'checkIn.status': 'pending' } });
    }
    catch (err) {
        console.warn('[Migration] Note on data normalization:', err.message);
    }
};
exports.normalizeExistingData = normalizeExistingData;
const seedDatabase = async () => {
    try {
        await (0, exports.normalizeExistingData)();
        const existingAdmin = await User_1.User.findOne({ email: 'admin@blooddonation.vn' });
        if (existingAdmin) {
        ...
        const event1 = await BloodDonationEvent_1.BloodDonationEvent.create({
            title: 'Chủ Nhật Đỏ 2026 — Giọt Hồng Trao Đi, Nụ Cười Ở Lại',
            description: 'Chương trình hiến máu nhân đạo thường niên quy mô lớn, tiếp nhận các nhóm máu phục vụ điều trị cấp cứu bệnh nhân tại Viện Huyết học và các bệnh viện tuyến đầu miền Bắc.',
        ...
        const event2 = await BloodDonationEvent_1.BloodDonationEvent.create({
            title: 'Hành Trình Đỏ 2026 — Kết Nối Dòng Máu Việt TP.HCM',
            description: 'Chiến dịch vận động hiến máu tình nguyện xuyên Việt tại TP. Hồ Chí Minh với thông điệp trao gửi yêu thương và tiếp sức người bệnh hiểm nghèo.',
        ...
        const event3 = await BloodDonationEvent_1.BloodDonationEvent.create({
            title: 'Ngày Hội Hiến Máu Tình Nguyện "Trái Tim Bách Khoa"',
            description: 'Ngày hội truyền thống của sinh viên và cán bộ giảng viên Đại học Bách Khoa Hà Nội nhằm lan tỏa tinh thần sẻ chia vì cộng đồng.',
        ...
        const reg1Id = new mongoose_1.default.Types.ObjectId();
        const qr1 = await (0, qrService_1.generateRegistrationQR)(reg1Id.toString(), user1._id.toString(), event1._id.toString());
        ...
        const reg2Id = new mongoose_1.default.Types.ObjectId();
        const qr2 = await (0, qrService_1.generateRegistrationQR)(reg2Id.toString(), user2._id.toString(), event1._id.toString());
        await Registration_1.Registration.create({
            _id: reg2Id,
            userId: user2._id,
            eventId: event1._id,
        ...
        const reg3Id = new mongoose_1.default.Types.ObjectId();
        const qr3 = await (0, qrService_1.generateRegistrationQR)(reg3Id.toString(), user3._id.toString(), event1._id.toString());
        await Registration_1.Registration.create({
            _id: reg3Id,
            userId: user3._id,
            eventId: event1._id,
        ...
        const reg4Id = new mongoose_1.default.Types.ObjectId();
        const qr4 = await (0, qrService_1.generateRegistrationQR)(reg4Id.toString(), user4._id.toString(), event1._id.toString());
        await Registration_1.Registration.create({
            _id: reg4Id,
            userId: user4._id,
            eventId: event1._id,
            ...
            screeningResult: {
                ...
                notes: 'Cần bác sĩ thăm khám và tư vấn trực tiếp tại sự kiện vì: Đang trong quá trình sử dụng thuốc điều trị / thực phẩm bổ sung.',
